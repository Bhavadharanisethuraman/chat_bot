...
# Upload document
def upload_document():
    file_path = st.file_uploader("Upload a PDF document", type="pdf")
...
def chatbot_conversation():
    ...
    st.write("Welcome to the loan application process!")
    
    user_input = st.text_input("Please provide your initial details:")
    if user_input:
        user_data = regex_extract(user_input)

        # Full Name extraction and splitting
        if "full_name" not in user_data:
            user_data["full_name"] = st.text_input("What is your full name?")
        ...
        while True:
            phone_number = st.text_input("Please provide your 10-digit contact number:")
            if validate_phone_number(phone_number):
                user_data["telephone"] = phone_number
                break
            st.error("Invalid phone number. Please enter a valid 10-digit contact number.")
        ...
        while True:
            ...
            if validate_email(email):
                ...
                break
            st.error("Invalid email address. Please enter a valid email in the format 'example@domain.com'.")

        # Ask for all other required fields dynamically
        for field in required_fields:
            if field not in user_data or not user_data[field]:
                if field == "promotion_applied":
                    promo_response = st.radio("Did you use a promotional code?", ["yes", "no"])
                    if promo_response == "yes":
                        promo_code = st.text_input("Please enter the promotional code:")
                        user_data["promotion_code"] = promo_code if promo_code in valid_promo_codes else "Invalid"
                elif "uploaded" in field:
                    uploaded_doc = upload_document()
                    user_data[field] = uploaded_doc if uploaded_doc else "No document uploaded"
                else:
                    prompt = random.choice(field_prompts[field])
                    user_data[field] = st.text_input(prompt)

        # Save to CSV
        df = pd.DataFrame([user_data])
        df.to_csv("loan_applications.csv", index=False)
        st.write("Thank you! Your loan application has been submitted.")
        st.write(df)
